...
They accrue locally in `.context/understandings/<subject>/<slug>.md` (gitignored — a subject folder groups a session's lessons, one file per unit), and reach the repository only through an approved publish of the units marked `scope: portable`. That split is the point: encoding stays cheap enough that agents propose freely, while shared memory stays small because review happens once, later, when it is clear which knowledge held up. Published Understandings travel between repositories over the same `ai-asset-sync` transport as skills and rules.
