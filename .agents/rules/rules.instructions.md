...
| `globs` | Cursor, Copilot | File pattern for auto-attach; quote the value |
...
Claude Code auto-loads all `.md` files in `.claude/rules/` (symlinked to `.agents/rules/`), so the frontmatter is informational for Claude.

### Scoping Guidelines

| Rule scope | Place in | `alwaysApply` | `globs` |
|------------|----------|---------------|---------|
...
|------|-----------|-----------|-------------|
| Claude Code | `.claude/rules/` (→ `.agents/rules/`) | `.md` | `paths` (optional) |
| Copilot | `.github/instructions` (path file → `../.agents/rules`) | `.instructions.md` | `globs`, `description` |
| Cursor | `.cursor/rules/` (→ `.agents/rules/`) | `.instructions.md` | `globs`, `description`, `alwaysApply` |
| Codex | any directory | `AGENTS.md` | none |

All four tools read from the same `.agents/rules/` source files via symlinks or committed path reference files.
