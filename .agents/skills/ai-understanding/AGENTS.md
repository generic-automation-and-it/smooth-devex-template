...
Encodes discovered knowledge as slug-folders under the gitignored `.context/understandings/`; the store is disposable by design, so export is the only path by which knowledge leaves a workspace — never weaken the `scope` gate that governs it.
...
- **Never default `--export` to include `repo-specific` units.** That flag is the only thing preventing a local quirk from being published with provenance that makes it look universally verified.
- **Never add remote fetching to this skill.** Remote import routes through `ai-asset-sync`'s manifest. Two transport implementations means two provenance models and two merge semantics.

## Architecture Decisions

### LADR-001 — Working store is local and disposable; export is the durability boundary
...
- **Decision:** The working store is `.context/understandings/` (local, gitignored, per-workspace). Reaching a tracked path is a separate, human-approved `--export` step, with `.agents/understandings/` as the default destination.
- **Consequences:** Encoding stays cheap and low-stakes, so agents propose freely. Review happens once, at export, when it is clear which knowledge held up. Trade-off: a workspace destroyed before its export loses its memory — accepted, because the alternative pollutes shared memory with unreviewed observations. The tracked destination is created on first export; nothing is seeded, so a repo that never exports carries no empty scaffolding.

### LADR-002 — `scope` is a first-class field, not a judgment made at export time

- **Date:** 2026-09-19 · **Status:** Accepted
- **Context:** Deciding portability when exporting means judging units long after the session that produced them, with none of the context that would reveal whether a behavior was general or an artifact of this repo's setup.
- **Decision:** Every Understanding declares `scope: portable | repo-specific` at encode time, while the evidence is fresh. Export filters on the field rather than re-deciding.
- **Consequences:** Export is mechanical and auditable. The judgment sits where the information is. Bias toward `repo-specific` when unclear: leaving knowledge behind costs one rediscovery, while publishing a false universal costs every consumer that acts on it.
...
- **Understandings are evidence; rules are orders.** The governance half of this design lives in `.github/instructions/meta/understandings.instructions.md`, not here, because inheriting must work at session start without anyone invoking the skill. Editing one side without the other splits the contract.
...
- **Imported units land as `confidence: observed`,** whatever they claimed at the source. They were verified against a setup that may differ.
...
| Date | Change | Ref |
|:-----|:-------|:----|
| 2026-09-19 | Initial version. Slug-folder store under `.context/`, generated reference index, `scope`-gated export, import via `ai-asset-sync`. | |
