...
- **Parsing is generic; validation is not.** A field shaped like one that already exists — a top-level list of `[[slug]]`s, say — needs no parser change at all, so the warning above is the wrong thing to check for that case and reading it as the whole story is how the real risk gets missed. `dangling_links` walks `links` and `dangling_inherited` walks `provenance.inherited`, each by name: an unrecognised list field parses, indexes, and is never checked, so a dangling reference inside it exits `0`. Extending the schema is therefore two separate questions — does `parse_frontmatter` need to grow, and does the field need a `dangling_*` check — and a field can need the second without the first.
